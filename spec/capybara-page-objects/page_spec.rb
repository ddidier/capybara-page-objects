...
describe CapybaraPageObjects::Page, :focused => true do
  ...
  describe 'Page#path' do
    ...
    it 'provides a DSL to set the raw path' do
      new_page('some/path').send(:raw_path).should eq('some/path')
    end
  ...
  shared_examples_for 'private examples for CapybaraPageObjects::Page' do
    ...
    describe 'Page#visit' do
      ...
      it 'visits the specified path' do
        page_class.visit(page_parameters).path.should eq(expected_path)
      end

      it 'returns the page instance' do
        page_class.visit(page_parameters).class.should eq(page_class)
      end
    end
  ...
  context 'when the path has no segment' do
    ...
    let(:page_class) { new_page_class(page_path) }
    let(:page_instance) { page_class.new(page_parameters) }
  ...
  context 'when the path has segments' do
    ...
    context 'and all segment parameters are missing' do
      it 'raises a MissingPathSegment error on creation' do
        expect { page_class.new }.to raise_error(CapybaraPageObjects::Page::MissingPathSegmentError,
    ...
    context 'and one segment parameters is missing' do
      it 'raises a MissingPathSegment error on creation' do
        expect { page_class.new(Hash[p1: 1]) }.to raise_error(CapybaraPageObjects::Page::MissingPathSegmentError,
  ...
  def new_page_class(path)
    Class.new(CapybaraPageObjects::Page) do
      self.path(path)
    end
  end
